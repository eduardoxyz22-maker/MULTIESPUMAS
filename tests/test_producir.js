...
(async () => {
  ...
  let r = await page.evaluate(() => {
    ...
    return { mes:R.mes.nombre, H:fila(window._K.H), S:fila(window._K.S), F:fila(window._K.F), M:fila(window._K.M), tot:tot, cubiertos:R.cubiertos, sinContar:R.sinContar,
  ...
  chk('totales por bloque: Heaven 1 · 9 · 19 (14+5); Sueña 0 · 0 · 7; sin asignar 4 · 4 · 1', r.tot.heaven.sem===1 && r.tot.heaven.quin===9 && r.tot.heaven.mes===19 && r.tot.heaven.mes1===14 && r.tot.heaven.mes2===5 && r.tot.suena.mes===7 && r.tot.suena.sem===0 && r.tot.otros.sem===4 && r.tot.otros.quin===4 && r.tot.otros.mes===1, JSON.stringify(r.tot));
  ...
  chk('el pie del bloque trae el total y el reparto por medida', r && /TOTAL/.test(r.t) && /por medida · 160x190/.test(r.t));
  ...
  r = await page.evaluate(() => {
    ...
    copiarProducir('suena','sem'); var nada=copiado;
    window.copyText=orig;
    return { mes:mes, sem:sem, nada:nada };
  });
  ...
  chk('«Ver también los cubiertos» agrega los que rotan pero no necesitan nada (y vuelve)', r.todos>r.soloAlgo && r.soloAlgo===4, JSON.stringify(r));
  r = await page.evaluate(() => {
