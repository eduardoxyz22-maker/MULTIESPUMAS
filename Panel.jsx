          ...
          <React.Fragment>
            ...
            <div className="drawer-body">
              ...
              <div className="dr-kpis" style={{ gridTemplateColumns: "1fr 1fr", marginBottom: 10 }}>
                ...
                <div className="dr-k" style={{ borderLeft: "3px solid var(--brand)" }}><div className="v" style={{ color: "var(--brand-d)" }}>{v.value ? fmtMoney(v.value) : "—"}</div><div className="l">Pipeline</div></div>
              </div>
      ...
      <div className="diag2">
        ...
        <div className="diag2-stats">
          ...
          <div className="diag2-stat" style={{ "--sc": "var(--green)" }}>
            ...
            <div><div className="diag2-v"><CountUp value={G.pipeline} fmt={fmtMoney} /></div><div className="diag2-l">Pipeline</div></div>
          </div>
        </div>
      </div>
...
function ViewResumen() {
  ...
  const spkCierres = (() => { const a = [0,0,0,0,0]; Object.values(D.weeklyClosures || {}).forEach(arr => arr.forEach((n,i) => { a[i] = (a[i]||0)+n; })); return a; })();
  const spkMoney = (() => { const avg = G.cierres ? G.cerrado / G.cierres : 0; return spkCierres.map(n => Math.round(n * avg)); })();
  return (
    <div className="view">
      <div className="rhero">
      ...
      <div>
        ...
        <div className="kpis" style={{ marginTop: 14, gridTemplateColumns: "repeat(auto-fill,minmax(168px,1fr))" }}>
          ...
          <Kpi l="Sin seguimiento" num={D.metrics?.backlogPct || 0} fmt={n => Math.round(n) + "%"} ac="#DC4046" ico="seguimiento" tip="Deals abiertos sin actividad en Kommo +72h." sub={<span><b style={{ color: "var(--red-ink)" }}>{D.metrics?.backlog || 0} {(D.metrics?.backlog || 0) === 1 ? "lead" : "leads"}</b> +72h</span>} />
